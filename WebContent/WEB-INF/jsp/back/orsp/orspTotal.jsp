...
%>
<%
	Object s = request.getAttribute("orspTotal");
%>
<!DOCTYPE HTML>
<html>
<head>
...
<div style="margin-left: 300px;">
                           <form target="_self" action="back/orspTotal" >
                                <input type="text" value="${orsp_date }" placeholder="请输入年份" name="orsp_date" style="float: left;width:120px;" class="form-control">
                                &nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" class="btn btn-outline btn-success" style="float: left;width:80px;" value="查询">
                            </form>
                        </div><br /><br />
     <!-- 为ECharts准备一个具备大小（宽高）的Dom -->
     <%if(s!=null){
    	 List<Map<String,Object>> orspTotal = ( List<Map<String,Object>>)s;
    	 if(orspTotal.size()>0){
    	 %>
    ...
    <script type="text/javascript">
    	var dates = []; 
    	var moneys = [];
    	<%for(int i=0;i<orspTotal.size();i++){%>
    		dates[<%=i%>] = "<%=orspTotal.get(i).get("d")%>";
    		moneys[<%=i%>] = <%=orspTotal.get(i).get("money")%>;
    	<%}%>
